...
public:
    ...
    ~segment() {//деструктов. об€зательно разрушение используемых динамических объектов
        delete P1;
        delete P2;
    }
    ...
    point* getFirstPoint() {//результаттом функции €вл€етс€ указатель на объект типа point
    ...
    //point * getMeedle(){//реализовать метод получени€ точки, котора€ €вл€етс€ серединой отрезка
    //    cout<<"("<<(P1->getX()+P2->getX())/2<<","<<(P1->getY()+P2->getY())/2<<")";
    //}

    bool pointOnLine(point* P) {//возвращает true если точка P лежит на пр€мой, на которой лежит отрезок
        double m, n, m1, n1;
        m = P1->getX() - P2->getX();
        n = P1->getY() - P2->getY();
        m1 = P1->getX() - P->getX();
...
public:
    ...
    ~figure() {//деструктов. об€зательно разрушение используемых динамических объектов
        delete pi;
    }
    void print() {
        for (int i = 0; i < dl; i++) {
